...
var connectDB = function () {
  MongoDriver.connect(Options.mongo.data, function (error, db) {
    if (!error) {
      ...
      throw new Error('ESMongoSync: Connection to database: ' + Options.mongo.data + ' failed!');
    }
  });
};
...
var tail = function () {
  ...
  Oplog = MongoOplog(Options.mongo.oplog).tail(function () {
 ...
 * init
 * @summary initializing package and setting up major connections
 * @param {Object} config - Config object to be used for specifying connection urls and batch count
 ...
 * @return {null} return null
 */
var init = function (config, watchers, esClient) {

  // validate arguments
  Validation.validateArgs(arguments);

  // continue package init
  Options.mongo = config.mongo;
  Options.elastic = config.elastic;
  Options.documentsInBatch = Number(Options.mongo.batch);
